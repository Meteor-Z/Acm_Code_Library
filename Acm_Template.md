...
$f(a\times b)=f(a)\times(b)这样的函数叫做积性函数(gcd(a,b)==1)$
...
给定$n$个元素，(编号是$1$~$n$),其中第$i$个元素是$a_i$,$b_i$,$c_i$三种属性，
...
$\text { 对于 } d \in[0, n) \text { ，求满足 } f(i)=d \text { 的 } i \text { 的数量。 }$

第一行两个整数$n,k$,表示元素数量和最大属性值
接下来$n$行，其中第$n$行包括三个整数$a_i,b_i,c_i$,分别表示第$i$个元素的三个属性值。
